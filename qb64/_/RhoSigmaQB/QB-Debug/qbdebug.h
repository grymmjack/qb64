...
int16 glbOpen = 0; // global file open flag
int16 ftrMode = 0; // active filter mode

int32 numIndt = 0;    // indent count
char *strIndt = "| "; // indent string

int16 fcdFlag = 0; // first call done flag
int16 eclFlag = 0; // empty close line flag
...
// ---------------------------------------------------------------------
void DebugInit(const char *logName, int16 filterMode) {
...
// ---------------------------------------------------------------------
int16 DebugOpen(const char *funcName) {
    int16 locOpen = glbOpen;         // remember global file open state
    ...
    if (!glbOpen) {                  // if not already open, then do it now
        ...
        if (!eclFlag) {
            for (int32 i = numIndt; i; i--) {fputs(strIndt, dbgFile);}
            fputs("|\n", dbgFile);
        }
        for (int32 i = numIndt; i; i--) {fputs(strIndt, dbgFile);}
        fprintf(dbgFile, "+-+ [%s]\n", funcName);
...
void DebugPrintf(const char *dbgFmtStr, ...) {
    ...
    int32 num = vsnprintf(0, 0, dbgFmtStr, args);
    va_end(args);
