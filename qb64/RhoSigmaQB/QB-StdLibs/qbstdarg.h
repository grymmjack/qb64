...
int16    fcdFlag = 0;   // first call done flag
...
{
    #ifdef RSQBSTDARG_DEBUG
    int16 locOpen = DebugOpen("ExitCStringCleanup (qbstdarg.h)");
    if (cMemBrain) {
    #endif
        while (cMemBrain) {
            #ifdef RSQBSTDARG_DEBUG
...
// ---------------------------------------------------------------------
ptrszint MakeCString(char *qbStr, int32 qbStrLen) {
    if (!fcdFlag) {
    ...
    *cMemPtr = cMemBrain; cMemBrain = (ptrszint) cMemPtr++; // memptr + linkptrsz = strptr (cMemPtr++)
    ...
    int16 locOpen = DebugOpen("MakeCString (qbstdarg.h)");
    ...
    #endif
    if (qbStr != 0) {
        char *seq; int32 len; // sequence temporaries
        for (int16 esc = 0; qbStrLen; qbStrLen--) {
            ...
            if (chr == '\\' && !esc) {esc = -1;}
                ...
                int32 sspo = (ptrszint) qbStr - qbStrOri - len; int32 sepo = sspo + len; // source start, end position
                int32 tspo = (ptrszint) cStr - (ptrszint) cMemPtr; // target start position
...
// ---------------------------------------------------------------------
int32 LenCString(ptrszint cStr) {
    if (!fcdFlag) {return 0;} // nothing there yet
    #ifdef RSQBSTDARG_DEBUG
    int16 locOpen = DebugOpen("LenCString (qbstdarg.h)");
    DebugPuts("given input...");
    DebugPrintf("pointer (cStr)  = 0x%016X", cStr);
    DebugPrintf("content (*cStr) = %s", cStr);
    #endif
    int32 len = strlen((const char*) cStr);
    #ifdef RSQBSTDARG_DEBUG
...
const char *ReadCString(ptrszint cStr) {
    ...
    #ifdef RSQBSTDARG_DEBUG
    int16 locOpen = DebugOpen("ReadCString (qbstdarg.h)");
    DebugPuts("given input...");
    DebugPrintf("pointer (cStr)  = 0x%016X", cStr);
    DebugPrintf("content (*cStr) = %s", cStr);
...
void FreeCString(ptrszint cStr) {
    ...
    #ifdef RSQBSTDARG_DEBUG
    int16 locOpen = DebugOpen("FreeCString (qbstdarg.h)");
    DebugPuts("given input...");
    DebugPrintf("pointer (cStr)  = 0x%016X", cStr);
    DebugPrintf("content (*cStr) = %s", cStr);
    DebugPuts("runtime variables...");
